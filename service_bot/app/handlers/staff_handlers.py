...
async def accept_order(callback: CallbackQuery):
    """Обработчик нажатия на кнопку принятия заказа"""
    order_number = callback.data.split("_")[1]
    await callback.answer("✅ Заказ принят")
    
    await callback.message.answer(f"✅ Заказ #{order_number} успешно принят!")

@router.callback_query(F.data.startswith("reject_"))
async def reject_order(callback: CallbackQuery):
    """Обработчик нажатия на кнопку отклонения заказа"""
    order_number = callback.data.split("_")[1]
    await callback.answer("❌ Заказ отклонен")
    
    await callback.message.answer(f"❌ Заказ #{order_number} отклонен!")
